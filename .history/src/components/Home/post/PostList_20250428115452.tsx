      ...
      {windowWidth > 642 ? (
        ...
        <div className="w-[100%] h-[100%] flex flex-col">
          ...
          {postListContents.map(post => (
            <div
              key={post.id}
              className="flex items-center justify-between h-[5.9rem] px-[1.6rem] py-[2.4rem] border-b-[0.1rem] border-[#EEEFF1]"
            >
              <div>
                <p className="text-[1.6rem] font-bold leading-[160%] tracking-[-0.3%]">예시</p>
                <div className="flex gap-[1.3rem]">
        <p className="text-[1.6rem] font-regular text-[#A7A9B4]">예시</p>
        <div className="flex gap-[0.5rem]">
          <img src="/icon/Comments.svg" alt="댓글아이콘" width="22.19" height="18" />
          <p className="text-[1.6rem] font-regular text-[#A7A9B4]">1</p>
        </div>
              </div>
            </div>
          ))}
